...
class TradeHandler:
    """
    TradeHandler – مدیریت منوی قابلیت‌های آینده (Up Next Features)
    """
    ...
    async def trade_menu(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """
        نمایش منوی قابلیت‌های آینده
        • کاربر را به بخش جدید هدایت می‌کند
        • توضیح مختصر دربارهٔ وضعیت فعلی این بخش
        • نمایش دکمه‌های Back و Exit
        """
        try:
            # ───➤ ست‌کردن state برای نمایش منوی قابلیت‌های آینده
            ...
            await self.error_handler.handle(
                update, context, e, context_name="upcoming_feature_menu"
            )
...
# TRADE_WALLET_ADDRESS = config.TRADE_WALLET_ADDRESS # This is the address where users send their payments when buying tokens.
...
# SUPPORT_USER_USERNAME = config.SUPPORT_USER_USERNAME
...
#                 txid=txid,
#                 to_address=TRADE_WALLET_ADDRESS,
